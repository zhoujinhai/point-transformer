...
class TransitionDownExport(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, nsample=16):
        super().__init__()
        self.stride, self.nsample = stride, nsample
        if stride != 1:
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            #     count += (o[i].item() - o[i-1].item()) // self.stride
            #     n_o.append(count) 
            # n_o = [o[0].item() // self.stride]
            # n_o = torch.cuda.IntTensor(n_o)
            # print("n_o  11111111: ", n_o)
            n_o = (o[0:1] // self.stride)    # for export
            # print("n_o  22222222: ", n_o) 
            idx = pointops.furthestsampling(p, o, n_o)  # (m)
            n_p = p[idx.long(), :]  # (m, 3)
            x = pointops.queryandgroup(self.nsample, p, n_p, x, None, o, n_o, use_xyz=True)  # (m, 3+c, nsample) 
            x = self.relu(self.bn(self.linear(x).transpose(1, 2).contiguous()))  # (m, c, nsample)
...
class TransitionUpExport(nn.Module):
    def __init__(self, in_planes, out_planes=None):
        super().__init__()
        if out_planes is None:
            self.linear1 = nn.Sequential(nn.Linear(2*in_planes, in_planes), nn.BatchNorm1d(in_planes), nn.ReLU(inplace=True))
    ...
    def forward(self, pxo1, pxo2=None):
        if pxo2 is None:
            _, x, o = pxo1  # (n, 3), (n, c), (b)  
            s = torch.zeros(1, dtype=torch.int64, device=o.device)
            e = o[0:1].to(torch.int64)
            cnt = e[0] - s[0]      
            # print(o, s, e, cnt)
            x_b = x[s[0]:e[0]]
            # print("xb: ", x_b)
            ...
            x1 = self.linear1(x1)
            x2 = self.linear2(x2) 
            x_inter = pointops.interpolation_export(p2, p1, x2, o2, o1)
            x = x1 + x_inter
        return x
...
class PointTransformerSegExport(nn.Module):
    def __init__(self, block, blocks, in_channels=6, n_cls=13):
        ...
        self.dec2 = self._make_dec(block, planes[1], 2, share_planes, nsample=nsample[1])  # fusion p3 and p2
        self.dec1 = self._make_dec(block, planes[0], 2, share_planes, nsample=nsample[0])  # fusion p2 and p1
        self.cls = nn.Sequential(nn.Linear(planes[0], planes[0]), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True), nn.Linear(planes[0], n_cls))
    ...
    def _make_dec(self, block, planes, blocks, share_planes=8, nsample=16, is_head=False):
        layers = []
        layers.append(TransitionUpExport(self.in_planes, None if is_head else planes * block.expansion))
        self.in_planes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.in_planes, self.in_planes, share_planes, nsample=nsample))
        return nn.Sequential(*layers)
    ...
    def forward(self, feats):
        ...
        # print("p0", p0.shape, x0.shape, o0, o0.shape)
        x0 = p0 if self.c == 3 else torch.cat((p0, x0), 1)
        p1, x1, o1 = self.enc1([p0, x0, o0]) 
        p2, x2, o2 = self.enc2([p1, x1, o1])
        p3, x3, o3 = self.enc3([p2, x2, o2])
